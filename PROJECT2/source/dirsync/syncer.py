...
Report the difference in `con`tent
...
class Syncer(object):
    ...
    def __init__(self, dir1, dir2,logfile_name, action, **options):
        ...
        if not os.path.isdir(self._dir1):
            raise ValueError("Error: Source directory does not exist. [Please make sure the directory is a valid directory]")
    ...
    def log(self, msg=''):
        ...
        logger.info(self.logger.info(msg)) 
    ...
    def report(self):
        ...
        self.log('%s finished in %s seconds at %s ' % (__pkg_name__, tt,date))
        self.log('%d directories parsed, %d files copied' %
                 (self._numdirs, self._numfiles))
        if self._numdelfiles:
            self.log('%d files were purged (removed).' % self._numdelfiles)
        if self._numdeldirs:
            self.log('%d directories were purged(removed).' % self._numdeldirs)
